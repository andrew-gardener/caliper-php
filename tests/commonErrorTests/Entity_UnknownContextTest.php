 ...
 * @requires PHP 5.6.28
 */
class Entity_UnknownContextTest extends CaliperErrorTestCase {
    function testError() {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('IMSGlobal\Caliper\entities\Entity::setContext: instance of Context expected');

        (new Person('https://example.edu/users/554433'))
            ->setContext('http://some.domain.com/not/a/caliper/context')
            ->setOtherIdentifiers([
    ...
    function testError2() {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('IMSGlobal\Caliper\context\Context::__construct: Invalid value. Valid values: CONTEXT');

        (new Person('https://example.edu/users/554433'))
            ->setContext(new Context('http://some.domain.com/not/a/caliper/context'))
            ->setOtherIdentifiers([
                (new Person('https://example.edu/users/998877')),
                (new SystemIdentifier('example.edu:71ee7e42-f6d2-414a-80db-b69ac2defd4', new SystemIdentifierType(SystemIdentifierType::LIS_SOURCED_ID))),
                (new SystemIdentifier('https://example.edu/users/554433', new SystemIdentifierType(SystemIdentifierType::LTI_USERID)))
